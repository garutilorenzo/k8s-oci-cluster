...
resource "oci_load_balancer_listener" "k8s_http_listener" {
  default_backend_set_name = oci_load_balancer_backend_set.k8s_http_backend_set.name
...
resource "oci_load_balancer_backend" "k8s_http_backend" {
  ...
  backendset_name  = oci_load_balancer_backend_set.k8s_http_backend_set.name
...
resource "oci_load_balancer_listener" "k8s_https_listener" {
  default_backend_set_name = oci_load_balancer_backend_set.k8s_https_backend_set.name
  load_balancer_id         = oci_load_balancer_load_balancer.k8s_public_lb.id
  ...
  ssl_configuration {
    certificate_name        = oci_load_balancer_certificate.k8s_https_certificate.certificate_name
...
resource "oci_load_balancer_backend" "k8s_https_backend" {
  ...
  count            = var.k8s_worker_pool_size
  backendset_name  = oci_load_balancer_backend_set.k8s_https_backend_set.name
  ip_address       = data.oci_core_instance.k8s_workers_instances_ips[count.index].private_ip
  load_balancer_id = oci_load_balancer_load_balancer.k8s_public_lb.id
  port             = var.extlb_listener_https_port
}