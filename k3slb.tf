...
resource "oci_load_balancer_backend_set" "k8s_kube_api_backend_set" {
  health_checker {
    protocol = "TCP"
    port     = var.kube_api_port
  }
  load_balancer_id = oci_load_balancer_load_balancer.k8s_load_balancer.id
  name             = "k8s__kube_api_backend_set"
  policy           = "ROUND_ROBIN"
}

resource "oci_load_balancer_backend" "k8s_kube_api_backend" {
  depends_on = [
    oci_core_instance_pool.k8s_servers,
  ]

  count            = var.k8s_server_pool_size
  backendset_name  = oci_load_balancer_backend_set.k8s_kube_api_backend_set.name
  ip_address       = data.oci_core_instance.k8s_servers_instances_ips[count.index].private_ip
  load_balancer_id = oci_load_balancer_load_balancer.k8s_load_balancer.id
  port             = var.kube_api_port
}